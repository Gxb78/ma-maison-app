/**
 * MAISON VITAL - GAMIFICATION ENGINE v2.1
...
export const BADGES_DATA = [
  ...
  },
  {
    ...
    icon: "fa-list-check",
    color: "text-blue-500",
    condition: (stats) => stats.sessionTasks >= 3,
  },
  {
    id: "badge_bricolo",
    name: "Bricoleur",
  ...
  },
  {
    ...
    condition: (stats) =>
      stats.healthScore >= 90 && stats.totalTasksCompleted >= 5,
  },
];
 ...
 * @param {Array} tasks - Liste des objets { item, status: 'ok'|'urgent'|'late' }.
 * @returns {number} Score de 0 à 100.
 */
export function calculateHealthScore(tasks) {
  if (!tasks || tasks.length === 0) return 100; // Pas de nouvelles, bonnes nouvelles
  ...
  tasks.forEach((task) => {
    ...
    let riskWeight = 1;
    // On sécurise l'accès à risk (si jamais l'item est mal formé)
    const riskTxt = (
      task.item.risk_description ||
      task.item.risk ||
      ""
    ).toLowerCase();

    if (
      riskTxt.includes("incendie") ||
      riskTxt.includes("légionelles") ||
      riskTxt.includes("santé")
    ) {
      riskWeight = 3; // Critique
    } else if (riskTxt.includes("panne") || riskTxt.includes("fuite")) {
      riskWeight = 2; // Moyen
    }
    ...
    // 'ok' n'ajoute aucune pénalité (Entropie = 0)
  });
...
export function checkBadges(lastTaskItem = null) {
  ...
  if (lastTaskItem) {
    ...
    // Gestion de la sauvegarde des coûts (si l'info est dispo)
    if (lastTaskItem.cost && lastTaskItem.cost > userStats.maxCostSaved) {
...
export function calculateTaskXP(item) {
  // Sécurité si les champs sont manquants
  const duration = item.duration_minutes || 10;
  const freq = item.frequency_months || 3;

  // Formule : Base + (Durée * 2) + Bonus pour les tâches rares (Fréquence > 6 mois)
  let xp = CONSTANTS.BASE_TASK_XP + duration * CONSTANTS.TIME_BONUS;

  if (freq >= 6) {
    xp += 50; // Bonus "Gros chantier"
  }

  // Si l'item a une récompense manuelle définie, on l'utilise comme plancher
  if (item.xp_reward && item.xp_reward > xp) {
    xp = item.xp_reward;
  }
